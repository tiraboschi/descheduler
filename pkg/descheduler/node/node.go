	...
	for _, taint := range nodeTaints {
		if taint.Key != taintKey {
			...
			if taint.Value != taintValue || taint.Effect != v1.TaintEffectPreferNoSchedule {
				newTaints = append(newTaints, v1.Taint{Key: taint.Key, Value: taint.Value, Effect: v1.TaintEffectPreferNoSchedule})
				updated = true
			} else {
				found = true
			}
...
// to update the node; otherwise, no API calls. Return error if any.
func RemoveSoftTaint(ctx context.Context, c clientset.Interface, node *v1.Node, taintKey, taintValue string) (bool, error) {
	newNode := node.DeepCopy()
	nodeTaints := newNode.Spec.Taints
	var newTaints []v1.Taint
	found := false
